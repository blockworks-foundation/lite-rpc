...
impl EpochCache {
    ...
    pub async fn bootstrap_epoch(rpc_client: &RpcClient) -> Result<EpochCache, ClientError> {
        ...
        panic!("SysvarAccountType::EpochSchedule can(t be deserilized. Eoch can't be calculated.");
    };
...
mod tests {
    ...
    fn test_epoch_calculus() {
        ...
        let slot1epoch = epoch_cache.get_epoch_at_slot(1);
        assert_eq!(
            Epoch {
                epoch: 0,
                slot_index: 1,
        ...
        let slot1epoch = epoch_cache.get_epoch_at_slot(999);
        assert_eq!(
            Epoch {
                epoch: 0,
                slot_index: 999,
        ...
        let slot1epoch = epoch_cache.get_epoch_at_slot(1001);
        assert_eq!(
            Epoch {
                epoch: 1,
                slot_index: 1,
        ...
        let slot1epoch = epoch_cache.get_epoch_at_slot(14031);
        assert_eq!(
            Epoch {
                epoch: 14,
                slot_index: 31,
