...
impl PrioritizationFeesHeap {
    ...
    pub async fn insert(&self, tx: SentTransactionInfo) {
        ...
        write_lock.signatures.insert(tx.signature.clone());
    ...
    pub async fn remove_expired_transactions(&self, current_blockheight: u64) {
        ...
        for (p, entry) in write_lock.map.iter_mut() {
            entry.retain(|x| {
                ...
                if !retain {
                    signatures_to_remove.push(x.signature.clone());
                }
                retain
            });
            if entry.is_empty() {
...
mod tests {
    ...
    pub async fn test_prioritization_heap() {
        ...
        let tx_0 = tx_creator("1".to_string(), 0);
        let tx_1 = tx_creator("2".to_string(), 10);
        let tx_3 = tx_creator("3".to_string(), 0);
        let tx_2 = tx_creator("4".to_string(), 100);
        ...
        let tx_0 = tx_creator("1".to_string(), 0);
        let tx_1 = tx_creator("2".to_string(), 10);
        let tx_2 = tx_creator("3".to_string(), 100);
        let tx_3 = tx_creator("4".to_string(), 0);
        let tx_4 = tx_creator("5".to_string(), 0);
        let tx_5 = tx_creator("6".to_string(), 1000);
        let tx_6 = tx_creator("7".to_string(), 10);
        p_heap.insert(tx_0.clone()).await;
        p_heap.insert(tx_1.clone()).await;
        p_heap.insert(tx_2.clone()).await;
        p_heap.insert(tx_3.clone()).await;
