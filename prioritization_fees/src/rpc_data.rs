...
impl PrioFeesStats {
    pub fn get_percentile(&self, percentile: f32) -> Option<(u64, u64)> {
        let index_tx = self.by_tx_percentiles.iter().enumerate().find(|(_, v)| **v==percentile).map(|(ix,_)| ix);
        let index_cu = self.by_cu_percentiles.iter().enumerate().find(|(_, v)| **v==percentile).map(|(ix,_)| ix);
