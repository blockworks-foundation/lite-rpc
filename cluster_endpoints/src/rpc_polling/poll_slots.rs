...
pub async fn poll_slots(
    ...
    tokio::spawn(poll_commitment_slots(rpc_client, commitment_config, slot_update_sx));
    loop {
        match tokio::time::timeout(AVERAGE_SLOT_CHANGE_TIME,
            slot_update_rx.recv(),
        )
        .await
        {
            Ok(Some(slot)) => {
                ...
                if slot > current_slot {
                    ...
                    sender.send(SlotNotification { processed_slot: current_slot, estimated_processed_slot: estimated_slot }).context("Cannot send slot notification")?;
                }
            }
            ...
            Err(err) => {
                ...
                sender.send(SlotNotification { processed_slot: current_slot, estimated_processed_slot: estimated_slot }).context("Connot send slot notification")?;
            }
        }
    }
}